...
const MONGO_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/market-rwanda';
...
async function scrubDatabase() {
  ...
  try {
    ...
    // 3. Scrub Seller Profiles (if any have placeholders)
    const sellers = db.collection('sellerprofiles');
    const sellerResult = await sellers.updateMany(
      { "documents.photo": { $regex: /placehold\.co/ } },
      { $set: { "documents.photo": UNSPLASH_PRODUCT } }
    );
    console.log(`Updated ${sellerResult.modifiedCount} seller documents.`);
